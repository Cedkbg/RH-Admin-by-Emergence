...
import { Plus, Calendar, User, Trash2, Pencil, Loader2 } from "lucide-react";
...
const Taches = () => {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    if (error) {
      setTasks(prev);
      toast.error("Impossible de déplacer la tâche", { description: error.message });
    } else {
    ...
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Tableau Kanban — glissez les cartes entre les colonnes</p>
        </div>
                        ...
                        <Draggable draggableId={t.id} index={idx} key={t.id}>
                          {(prov, snap) => (
                            <Card
                              ...
                              className={`cursor-grab active:cursor-grabbing transition-shadow ${snap.isDragging ? "shadow-lg ring-2 ring-primary" : "hover:shadow-md"}`}
                              onClick={() => openEdit(t)}
                            >
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Que faut-il faire ?" />
            </div>
            <div>
